var g,g2,el1,el2,my={}
var version='0.936'
var Base64={_keyStr:"ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/=",encode:function(e){var t="";var n,r,i,s,o,u,a;var f=0;e=Base64._utf8_encode(e);while(f<e.length){n=e.charCodeAt(f++);r=e.charCodeAt(f++);i=e.charCodeAt(f++);s=n>>2;o=(n&3)<<4|r>>4;u=(r&15)<<2|i>>6;a=i&63;if(isNaN(r)){u=a=64}else if(isNaN(i)){a=64}t=t+this._keyStr.charAt(s)+this._keyStr.charAt(o)+this._keyStr.charAt(u)+this._keyStr.charAt(a)}
...
return t}}
function playerMain(filename,w,h,rel='../',voiceType=''){my.devQ=(document.domain=='localhost')?true:false
filename=filename.split('$').join('/')
console.log('playerMain',filename,w,h,voiceType)
var iframe=document.createElement('iframe');iframe.style.width=w+'px';iframe.style.height=h+'px';iframe.style.border='2px solid #eef0ff';iframe.style.borderRadius='23px';iframe.style.display='block';iframe.style.margin='auto';iframe.scrolling='no';var html='<!doctype html><html><head>';html+='<meta http-equiv="content-type" content="text/html; charset=utf-8" />';if(voiceType=='AWS')html+='<script src="https://sdk.amazonaws.com/js/aws-sdk-2.410.0.min.js"></script>'
html+='<script src="'+rel+'images/player.js" type="text/javascript"></script>';html+='<link rel="stylesheet" type="text/css" href="'+rel+'stylejs.css" />';html+='</head>';html+='<body style="margin: 0; padding: 0;">';html+=`<script type="text/javascript">playerStt("${filename}",${w},${h},"${voiceType}");</script>`
...
if(!my.voiceQ)return
console.log("SAY",cmd.txt);if(false){var utterance=new SpeechSynthesisUtterance(cmd.txt);utterance.rate=0.9;utterance.pitch=1.1;utterance.voice=my.voice.getVoices()[2];my.voice.speak(utterance);}
var s='<speak><prosody rate="90%">'+cmd.txt+'</prosody></speak>';my.voice.speak(s);}},preset:{n:0,run(cmd){var preName=cmd.txt
...
var xhr;xhr=new XMLHttpRequest();xhr.onreadystatechange=handleStateChange;xhr.open("GET",url,true);xhr.send();function handleStateChange(){if(xhr.readyState===4){console.log('getFileFromServer status:',xhr.status,url)
...
class VoiceAWS{constructor(){this.speechParams={OutputFormat:"mp3",SampleRate:"16000",Text:"This is a test",TextType:"ssml",VoiceId:"Salli",};this.polly=new AWS.Polly({apiVersion:'2016-06-10'});this.signer=new AWS.Polly.Presigner(this.speechParams,this.polly)
var elementId="audioElement"+new Date().valueOf().toString();this.audioElement=document.createElement('audio');this.audioElement.setAttribute("id",elementId);document.body.appendChild(this.audioElement);console.log('VoiceAWS this.audioElement',this.audioElement)
this.speaking=false
this.pending=false
this.cachQ=true}
speakTest(s){this.speechParams.Text=s
this.signer.getSynthesizeSpeechUrl(this.speechParams,function(error,url){if(error){document.getElementById('result').innerHTML=error;}else{console.log('url=',url);document.getElementById('audioSource').src=url;document.getElementById('audioPlayback').load();document.getElementById('result').innerHTML="Speech ready to play.";}});}
speak(s){this.speaking=true;var me=this
return new Promise(function(successCallback,errorCallback){me.getAudio(s).then(function(result){me.playAudio(result);}).then(successCallback);});}
getAudio(s){if(!this.cachQ){return this.requestSpeechFromAWS(s);}else{var zizzy=this.requestSpeechFromLocalCache(s)
console.log('getAudio',s,zizzy)
if(zizzy===null){return this.requestSpeechFromAWS(s);}else{return zizzy}}}
playAudio(audioStream){console.log('playAudio',audioStream)
var me=this
return new Promise(function(success,error){var uInt8Array=new Uint8Array(audioStream);var arrayBuffer=uInt8Array.buffer;var blob=new Blob([arrayBuffer]);var url=URL.createObjectURL(blob);me.audioElement.src=url;me.audioElement.addEventListener("ended",function(){console.log('playAudio ended')
me.speaking=false;});me.audioElement.play();});}
playAudio64(base64){console.log('playAudio64',base64)
var me=this
return new Promise(function(success,error){var stream=Base64.encode(base64)
var uInt8Array=new Uint8Array(stream);var arrayBuffer=uInt8Array.buffer;var blob=new Blob([arrayBuffer]);var url=URL.createObjectURL(blob);me.audioElement.src=url;me.audioElement.addEventListener("ended",function(){console.log('playAudio64 ended')
me.speaking=false;});me.audioElement.play();});}
b64toBlob(b64Data,contentType='',sliceSize=512){const byteCharacters=b64Data;const byteArrays=[];for(let offset=0;offset<byteCharacters.length;offset+=sliceSize){const slice=byteCharacters.slice(offset,offset+sliceSize);const byteNumbers=new Array(slice.length);for(let i=0;i<slice.length;i++){byteNumbers[i]=slice.charCodeAt(i);}
...
requestSpeechFromAWS(s){console.log('requestSpeechFromAWS',s)
var me=this
return new Promise(function(successCallback,errorCallback){me.speechParams.Text=s
me.polly.synthesizeSpeech(me.speechParams,function(error,data){if(error){errorCallback(error)}else{me.saveSpeechToLocalCache(s,data.AudioStream);successCallback(data.AudioStream);}});});}
requestSpeechFromLocalCache(s){console.log('requestSpeechFromLocalCache',s)
var audioDictionary=localStorage.getItem("voiceCache");if(audioDictionary===null){return null;}
var audioStreamArray=JSON.parse(audioDictionary);var audioStream=audioStreamArray.filter(function(record){return record.Message===s;})[0];if(audioStream===null||typeof audioStream==='undefined'){console.log('requestSpeechFromLocalCache FAIL')
return null;}else{console.log('requestSpeechFromLocalCache success',this.speaking)
return new Promise(function(successCallback,errorCallback){console.log('requestSpeechFromLocalCache',audioStream)
successCallback(JSON.parse(audioStream.AudioStream).data);});}}
saveSpeechToLocalCache(s,audioStream){console.log('saveSpeechToLocalCache',audioStream)
var record={Message:s,AudioStream:JSON.stringify(audioStream),base64:Base64.encode(audioStream)};var localPlaylist=JSON.parse(localStorage.getItem("voiceCache"));console.log('record',record)
if(localPlaylist===null){localPlaylist=[];localPlaylist.push(record);}else{localPlaylist.push(record);}
localStorage.setItem("voiceCache",JSON.stringify(localPlaylist));}
cancel(){return ''}}
